...
Ret etk_power_button_listener(void* user_data, void* obj){
	EtkWidget *thiz=obj;
...
EtkWidget *etk_power_create(e32 x,e32 y,e32 width,e32 height){
	...
	EtkWidget *thiz=etk_create_window(x,y,width,height,ETK_WIDGET_CHILD|ETK_WIDGET_WINDOW);
	...
	for(i=0;i<=POWER_MAX_NR;i++){
		...
		etk_button_set_clicked_listener(power->buttons[i],etk_power_button_listener,power->buttons[i]);
		wx=wx+xsize+xspan;
			//printf("addr[%d] :%x\n",i,power->buttons[i]);
		if((i+1)%4==0){
			wy=wy+ysize+yspan;

...
int etk_app_main( int argc, char* argv[] ){
	...
	e8 button_name[20][10]={"时钟","Vedio","Temp","Note","Shut","Help","Mine","Block","Photo"};
	...
	etk_input_method_manager_register(etk_default_input_method_manager(),etk_input_method_pinyin_create());
...

	{
		EtkWidget *entry;
		EtkSource *timer;
		wins[0]=etk_create_window(20,20,200,150,ETK_WIDGET_CHILD|ETK_WIDGET_WINDOW);
		etk_widget_append_child(desktop,wins[0]);
		etk_widget_set_text(wins[0],"gaga test");
		timer=etk_source_timer_create(100,timer2_listener,wins[0]);
		etk_sources_manager_add(etk_default_sources_manager(),timer);
		etk_canvas_draw_char(wins[0]->canvas,50,50,'嘎');
		etk_canvas_draw_string(wins[0]->canvas,50,80,"嘎嘎嘎");
		entry=etk_entry_create(wins[0],20,30,80,20);
	}
	{
		EtkWidget *entry;
		EtkSource *timer;
	...
	for(i=0;i<2;i++){
		...
		wins[0]=etk_temhum_create(10,50,200,290);
		etk_widget_append_child(desktop,wins[0]);
		wins[1]=(EtkWidget*)etk_light_create(100,100,250,180);
		etk_widget_append_child(desktop,wins[1]);
		wins[2]=etk_status_create(200,100,200,150);
		etk_widget_append_child(desktop,wins[2]); 
		wins[3]=etk_power_create(300,200,250,200);
		etk_widget_append_child(desktop,wins[3]);
		
	/*
		t=etk_app_clock(490,30,150,150);
			etk_widget_append_child(desktop,t);
	
		*/
	
	
		t=etk_ajust_create(300,300,200,150);
		etk_widget_append_child(desktop,t);
...
Ret timer2_listener(void *data){
	...
	//etk_default_wnd_manager_update_widget_rect(etk_default_wnd_manager(),wii,&wii->rect);
	return RET_OK;
}
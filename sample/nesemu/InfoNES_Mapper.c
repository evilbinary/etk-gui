...
u8  Map4_Regs[ 8 ];
u32 Map4_Rom_Bank;
u32 Map4_Prg0, Map4_Prg1;
u32 Map4_Chr01, Map4_Chr23;
u32 Map4_Chr4, Map4_Chr5, Map4_Chr6, Map4_Chr7;
...
u8 Map5_Wram[ 0x2000 * 8 ];
u8 Map5_Ex_Ram[ 0x400 ]; 
u8 Map5_Ex_Vram[ 0x400 ];
u8 Map5_Ex_Nam[ 0x400 ];
...
u8 Map5_Wram_Protect0;
u8 Map5_Wram_Protect1;
u8 Map5_Prg_Size;
u8 Map5_Chr_Size;
u8 Map5_Gfx_Mode;
...
u8 Map5_ReadApu( u16 wAddr )
{
  ...
  switch ( wAddr )
  {
    ...
    case 0x5205:
      byRet = (u8)( ( Map5_Value0 * Map5_Value1 ) & 0x00ff );
      break;

    case 0x5206:
      byRet = (u8)( ( ( Map5_Value0 * Map5_Value1 ) & 0xff00 ) >> 8 );
      break;
...
/*-------------------------------------------------------------------*/
void Map5_Sram( u16 wAddr, u8 byData )
{
...
void Map5_RenderScreen( u8 byMode )
{
  ...
  {
    case 0:
      dwPage[ 7 ] = ( (u32)Map5_Chr_Reg[7][byMode] << 3 ) % ( NesHeader.byVRomSize << 3 );
    ...
    case 1:
      dwPage[ 3 ] = ( (u32)Map5_Chr_Reg[3][byMode] << 2 ) % ( NesHeader.byVRomSize << 3 );
      dwPage[ 7 ] = ( (u32)Map5_Chr_Reg[7][byMode] << 2 ) % ( NesHeader.byVRomSize << 3 );
    ...
    case 2:
      dwPage[ 1 ] = ( (u32)Map5_Chr_Reg[1][byMode] << 1 ) % ( NesHeader.byVRomSize << 3 );
      dwPage[ 3 ] = ( (u32)Map5_Chr_Reg[3][byMode] << 1 ) % ( NesHeader.byVRomSize << 3 );
      dwPage[ 5 ] = ( (u32)Map5_Chr_Reg[5][byMode] << 1 ) % ( NesHeader.byVRomSize << 3 );
      dwPage[ 7 ] = ( (u32)Map5_Chr_Reg[7][byMode] << 1 ) % ( NesHeader.byVRomSize << 3 );
    ...
    default:
      dwPage[ 0 ] = (u32)Map5_Chr_Reg[0][byMode] % ( NesHeader.byVRomSize << 3 );
      dwPage[ 1 ] = (u32)Map5_Chr_Reg[1][byMode] % ( NesHeader.byVRomSize << 3 );
      dwPage[ 2 ] = (u32)Map5_Chr_Reg[2][byMode] % ( NesHeader.byVRomSize << 3 );
      dwPage[ 3 ] = (u32)Map5_Chr_Reg[3][byMode] % ( NesHeader.byVRomSize << 3 );
      dwPage[ 4 ] = (u32)Map5_Chr_Reg[4][byMode] % ( NesHeader.byVRomSize << 3 );
      dwPage[ 5 ] = (u32)Map5_Chr_Reg[5][byMode] % ( NesHeader.byVRomSize << 3 );
      dwPage[ 6 ] = (u32)Map5_Chr_Reg[6][byMode] % ( NesHeader.byVRomSize << 3 );
      dwPage[ 7 ] = (u32)Map5_Chr_Reg[7][byMode] % ( NesHeader.byVRomSize << 3 );
...
void Map6_Apu( u16 wAddr, u8 byData )
{
  switch ( wAddr )
  {
    ...
    case 0x4502:
      Map6_IRQ_Cnt = ( Map6_IRQ_Cnt & 0xff00 ) | (u32)byData;
      break;

    case 0x4503:
      Map6_IRQ_Cnt = ( Map6_IRQ_Cnt & 0x00ff ) | ( (u32)byData << 8 );
      Map6_IRQ_Enable = 1;
      break;
  }
}
...
{
  u8 byPrgBank = ( byData & 0x3c ) >> 2;
  u8 byChrBank = byData & 0x03;
...
{
  u8 byPrgBank = ( byData & 0xf8 ) >> 3;
  u8 byChrBank = byData & 0x07;
...
{
  u8 byPrgBank = ( byData & 0x01 ) << 2;
  u8 byChrBank = ( ( byData & 0x70 ) >> 4 ) << 3;
...
void Map16_Write( u16 wAddr, u8 byData )
{
  switch ( wAddr & 0x000f )
  {
    ...
    case 0x0009:
      ...
      case 0x000c:
        Map16_IRQ_Latch = ( (u32)byData << 8 ) | ( Map16_IRQ_Latch & 0x00ff );
        break;
...
void Map17_Apu( u16 wAddr, u8 byData )
{
  switch ( wAddr )
  {
    ...
    case 0x4503:
      Map17_IRQ_Latch = ( Map17_IRQ_Latch & 0x00ff ) | ( (u32)byData << 8 );
...
void Map19_Init()
{
  ...
  if ( NesHeader.byVRomSize > 0 )
  {
    u32 dwLastPage = (u32)NesHeader.byVRomSize << 3;
...
void Map19_Apu( u16 wAddr, u8 byData )
{
  switch ( wAddr & 0xf800 )
  {
    ...
    case 0x5800:  /* $5800-5fff */
      Map19_IRQ_Cnt = ( Map19_IRQ_Cnt & 0x00ff ) | ( (u32)( byData & 0x7f ) << 8 );
...
u8 Map19_ReadApu( u16 wAddr )
{
  switch ( wAddr & 0xf800 )
  {
    ...
    case 0x5000:  /* $5000-57ff */
      return (u8)(Map19_IRQ_Cnt & 0x00ff );

    case 0x5800:  /* $5800-5fff */
      return (u8)( (Map19_IRQ_Cnt & 0x7f00) >> 8 );
 ...
 *
 */
  u8 Map19_IRQ_Timing = 113;

  if ( Map19_IRQ_Enable )
  {
    if ( Map19_IRQ_Cnt >= (u32)(0x7fff - Map19_IRQ_Timing) )
    {
 ...
 */
  if ( Map33_IRQ_Enable )
  {
    ...
    {
      if ( PPU_Scanline == (u16)( 0xff - Map33_IRQ_Cnt ) )
      {
        IRQ_REQ;
        Map33_IRQ_Cnt = 0;
        Map33_IRQ_Enable = 0;
...
u8  Map44_Regs[ 8 ];
u32 Map44_Rom_Bank;
u32 Map44_Prg0, Map44_Prg1;
u32 Map44_Chr01, Map44_Chr23;
u32 Map44_Chr4, Map44_Chr5, Map44_Chr6, Map44_Chr7;
...
u8  Map47_Regs[ 8 ];
u32 Map47_Rom_Bank;
u32 Map47_Prg0, Map47_Prg1;
u32 Map47_Chr01, Map47_Chr23;
u32 Map47_Chr4, Map47_Chr5, Map47_Chr6, Map47_Chr7;
...
u8  Map49_Regs[ 3 ];
u32 Map49_Prg0, Map49_Prg1;
u32 Map49_Chr01, Map49_Chr23;
u32 Map49_Chr4, Map49_Chr5, Map49_Chr6, Map49_Chr7;
...
{
  u32 dwBank0, dwBank1, dwBank2, dwBank3;
...
void Map65_Write( u16 wAddr, u8 byData )
{
  switch ( wAddr )
  {
    ...
    case 0x9005:
      Map65_IRQ_Latch = ( Map65_IRQ_Latch & 0x00ff ) | ((u32)byData << 8 );
      break;

    case 0x9006:
      Map65_IRQ_Latch = ( Map65_IRQ_Latch & 0xff00 ) | (u32)byData;
      break;
  ...
  switch ( wAddr )
  {
    ...
    case 0xA000:
      switch ( Map69_Regs[ 0 ] )
      {
        ...
        case 0x0e:
          Map69_IRQ_Cnt = ( Map69_IRQ_Cnt & 0xff00) | (u32)byData;
          break;

        case 0x0f:
          Map69_IRQ_Cnt = ( Map69_IRQ_Cnt & 0x00ff) | ( (u32)byData << 8 );
          break;
      }
      break;
  }
...
{
  u8 byChrBank = byData & 0x0f;
  u8 byPrgBank = ( byData & 0x70 ) >> 4;
...
{
  u8 byRomBank = byData & 0x07;
  u8 byChrBank = ( byData & 0xf0 ) >> 4;
...
void Map83_Write( u16 wAddr, u8 byData )
{
  ...
  switch( wAddr )
  {
    ...
    case 0x8200:
      Map83_IRQ_Cnt = ( Map83_IRQ_Cnt & 0xFF00 ) | (u32)byData;
      break;

    case 0x8201:
      Map83_IRQ_Cnt = ( Map83_IRQ_Cnt & 0x00FF ) | ((u32)byData << 8);
...
void Map89_Write( u16 wAddr, u8 byData )
{
  ...
  {
    u8 byPrgBank = (byData & 0x70) >> 4;
    u8 byChrBank = ((byData & 0x80) >> 4) | (byData & 0x07);
...
u8 Map90_Prg_Reg[ 4 ];
u8 Map90_Chr_Low_Reg[ 8 ];
u8 Map90_Chr_High_Reg[ 8 ];
u8 Map90_Nam_Low_Reg[ 4 ];
u8 Map90_Nam_High_Reg[ 4 ];

u8 Map90_Prg_Bank_Size;
u8 Map90_Prg_Bank_6000;
u8 Map90_Prg_Bank_E000;
u8 Map90_Chr_Bank_Size;
u8 Map90_Mirror_Mode;
u8 Map90_Mirror_Type;
...
u8 Map90_ReadApu( u16 wAddr )
{
  if ( wAddr == 0x5000 )
  {
    return ( u8 )( ( Map90_Value1 * Map90_Value2 ) & 0x00ff );
  } else {
    return ( u8 )( wAddr >> 8 );
  }
}
...
void Map90_Sync_Mirror( void )
{
  ...
  {
    dwNamBank[ byPage ] = 
      ( (u32)Map90_Nam_High_Reg[ byPage ] << 8 ) | (u32)Map90_Nam_Low_Reg[ byPage ];
  }
...
void Map90_Sync_Chr_Banks( void )
{
  ...
  {
    dwChrBank[ byPage ] =
      ( (u32)Map90_Chr_High_Reg[ byPage ] << 8 ) | (u32)Map90_Chr_Low_Reg[ byPage ];
  }
...
u8  Map95_Regs[ 1 ];
u32 Map95_Prg0, Map95_Prg1;
u32 Map95_Chr01, Map95_Chr23;
u32 Map95_Chr4, Map95_Chr5, Map95_Chr6, Map95_Chr7;
...
u8  Map112_Regs[8];
u32 Map112_Prg0,Map112_Prg1;
u32 Map112_Chr01,Map112_Chr23,Map112_Chr4,Map112_Chr5,Map112_Chr6,Map112_Chr7;

u8  Map112_IRQ_Enable;  /* IRQs enabled */
u8  Map112_IRQ_Cnt;     /* IRQ scanline counter, decreasing */
u8  Map112_IRQ_Latch;   /* IRQ scanline counter latch */
...
u8  Map114_Regs[ 8 ];
u32 Map114_Prg0, Map114_Prg1;
u32 Map114_Chr01, Map114_Chr23;
u32 Map114_Chr4, Map114_Chr5, Map114_Chr6, Map114_Chr7;

u8 Map114_IRQ_Enable;
u8 Map114_IRQ_Cnt;
u8 Map114_IRQ_Latch;
...
u8 Map117_IRQ_Line;
u8 Map117_IRQ_Enable1;
u8 Map117_IRQ_Enable2;
...
u8  Map118_Regs[ 8 ];
u32 Map118_Prg0, Map118_Prg1;
u32 Map118_Chr0, Map118_Chr1, Map118_Chr2, Map118_Chr3;
u32 Map118_Chr4, Map118_Chr5, Map118_Chr6, Map118_Chr7;

u8 Map118_IRQ_Enable;
u8 Map118_IRQ_Cnt;
u8 Map118_IRQ_Latch;
...
void Map122_Sram( u16 wAddr, u8 byData )
{
  if ( wAddr == 0x6000 )
  {
    u8 byChrBank0 = byData & 0x07;
    u8 byChrBank1 = ( byData & 0x70 ) >> 4;
...
u8 Map160_IRQ_Enable;
u8 Map160_IRQ_Cnt;
u8 Map160_IRQ_Latch;
u8 Map160_Refresh_Type;
...
void Map243_Apu( u16 wAddr, u8 byData )
{
  ...
  else if ( wAddr == 0x4101 )
  {
    ...
    {
      u8 chr_bank = ( Map243_Regs[2] + Map243_Regs[3] ) >> 1;
    ...
    {
      u8 chr_bank = Map243_Regs[1] + Map243_Regs[2] + Map243_Regs[3];

...
Ret etk_wnd_manager_set_active_widget(EtkWndManager* thiz,EtkWidget* active){
	...
	if(priv->active_widget!=NULL&&(priv->active_widget->type&ETK_WIDGET_WINDOW)){
		if(etk_window_has_head(priv->active_widget)){
	...
	if(active->type&ETK_WIDGET_WINDOW){
		if(etk_window_has_head(active)){
			EtkRect t,r;
			t=etk_window_get_head_rect(priv->active_widget);
			h=priv->active_widget->canvas->bitmap;
			etk_color_init(&color,BLUE,ETK_PIXEL_BGR24);//RGB24_2_RGB555(0x4f,0x4f,0x4f)
			for(j=t.y;j<t.height;j++){
				etk_bitmap_hline(h,t.x,j,t.width,color.color);
			}
			r=etk_window_get_close_rect(priv->active_widget);
			...
			etk_canvas_hline(active->canvas,2,t.y+t.height-3,active->rect.width-5,GRAY);
...
EtkWndManager* etk_default_wnd_manager_create(EtkMainLoop* main_loop){
	...
	if(thiz!=NULL){
		...
		etk_set_source_default(etk_source_default_create((EtkOnEvent)etk_wnd_manager_default_dispatch_event, thiz));
		etk_sources_manager_add(etk_default_sources_manager(), etk_get_default_source());
		thiz->winnum=0;
		thiz->windows=NULL;
		thiz->topwin=NULL;
		thiz->add=etk_default_wnd_manager_add;
